...
/**
 * CubePoints logs only to show the author data in author.php
 */

	
 function cp_show_logs_author($type='all', $limit=10, $datatables=true){
  ...
  $limitq = '';  
	...
	if($type!='all'){
		$uid = (int) $type;
		$q = ' WHERE `uid` = ' . $uid . ' AND `type` != "dailypoints" ';
	}
	if($limit>0){
		$limitq = 'LIMIT '.(int) $limit;
	}
	$results = $wpdb->get_results( 'SELECT * FROM `'.CP_DB.'` '.$q.'ORDER BY timestamp DESC ' . $limitq );
?>
	
	<table id="cp_logs_table" class="widefat<?php echo $datatables?' datatables':''; ?>">
		<thead><tr><th scope="col"><?php _e('Points','cp'); ?></th><th scope="col"><?php _e('Description','cp'); ?></th><th scope="col"><?php _e('Time','cp'); ?></th></tr></thead>
		<tfoot><tr><th scope="col"><?php _e('Points','cp'); ?></th><th scope="col"><?php _e('Description','cp'); ?></th><th scope="col"><?php _e('Time','cp'); ?></th></tr></tfoot>
